...
export class NoteService {
  ...
  protected convertDateFromClient(note: INote): INote {
    const copy: INote = Object.assign({}, note, {
      creationDate: note.creationDate && note.creationDate.isValid() ? note.creationDate.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((note: INote) => {
        note.creationDate = note.creationDate ? moment(note.creationDate) : undefined;
      });
    }
    return res;
  }
